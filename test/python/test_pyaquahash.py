import pyaquahash
from random import randint

def test_get_cache_size_not_None():
    for _ in range(100):
        block_num = randint(0,12456789)
        out = pyaquahash.get_cache_size(block_num)
        assert out != None

def test_get_full_size_not_None():
    for _ in range(100):
        block_num = randint(0,12456789)
        out = pyaquahash.get_full_size(block_num)
        assert out != None

def test_get_cache_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pyaquahash.get_cache_size(block_num)
        out2 = pyaquahash.get_cache_size((block_num // pyaquahash.EPOCH_LENGTH) * pyaquahash.EPOCH_LENGTH)
        assert out1 == out2

def test_get_full_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pyaquahash.get_full_size(block_num)
        out2 = pyaquahash.get_full_size((block_num // pyaquahash.EPOCH_LENGTH) * pyaquahash.EPOCH_LENGTH)
        assert out1 == out2

# See light_and_full_client_checks in test.cpp
def test_mkcache_is_as_expected():
    actual = pyaquahash.mkcache_bytes(
        1024,
        "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~").encode('hex')
    expected = ""
    assert actual == expected

def test_calc_dataset_is_not_None():
    cache = pyaquahash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    assert pyaquahash.calc_dataset_bytes(1024 * 32, cache) != None

def test_light_and_full_agree():
    cache = pyaquahash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    full_size = 1024 * 32
    header = "~~~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~"
    light_result = pyaquahash.hashimoto_light(full_size, cache, header, 0)
    dataset = pyaquahash.calc_dataset_bytes(full_size, cache)
    full_result = pyaquahash.hashimoto_full(dataset, header, 0)
    assert light_result["mix digest"] != None
    assert len(light_result["mix digest"]) == 32
    assert light_result["mix digest"] == full_result["mix digest"]
    assert light_result["result"] != None
    assert len(light_result["result"]) == 32
    assert light_result["result"] == full_result["result"]

def int_to_bytes(i):
    b = []
    for _ in range(32):
        b.append(chr(i & 0xff))
        i >>= 8
    b.reverse()
    return "".join(b)

def test_mining_basic():
    easy_difficulty = int_to_bytes(2**256 - 1)
    assert easy_difficulty.encode('hex') == 'f' * 64
    cache = pyaquahash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    full_size = 1024 * 32
    header = "~~~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~"
    dataset = pyaquahash.calc_dataset_bytes(full_size, cache)
    # Check type of outputs
    assert type(pyaquahash.mine(dataset,header,easy_difficulty)) == dict
    assert type(pyaquahash.mine(dataset,header,easy_difficulty)["nonce"]) == long
    assert type(pyaquahash.mine(dataset,header,easy_difficulty)["mix digest"]) == str
    assert type(pyaquahash.mine(dataset,header,easy_difficulty)["result"]) == str

def test_mining_doesnt_always_return_the_same_value():
    easy_difficulty1 = int_to_bytes(int(2**256 * 0.999))
    # 1 in 1000 difficulty
    easy_difficulty2 = int_to_bytes(int(2**256 * 0.001))
    assert easy_difficulty1 != easy_difficulty2
    cache = pyaquahash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    full_size = 1024 * 32
    header = "~~~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~"
    dataset = pyaquahash.calc_dataset_bytes(full_size, cache)
    # Check type of outputs
    assert pyaquahash.mine(dataset, header, easy_difficulty1)['nonce'] != pyaquahash.mine(dataset, header, easy_difficulty2)['nonce']

def test_get_seedhash():
    assert pyaquahash.get_seedhash(0).encode('hex') == '0' * 64
    import hashlib, sha3
    expected = pyaquahash.get_seedhash(0)
    #print "checking seed hashes:",
    for i in range(0, 30000*2048, 30000):
        #print i // 30000,
        assert pyaquahash.get_seedhash(i) == expected
        expected = hashlib.sha3_256(expected).digest()
