...
mkcache_bytes(PyObject *self, PyObject *args) {
    ...
    aquahash_light_t L = aquahash_light_new(block_number);
...
calc_dataset_bytes(PyObject *self, PyObject *args) {
    ...
    if (cache_size % AQUAHASH_HASH_BYTES != 0) {
        char error_message[1024];
        sprintf(error_message, "The size of the cache must be a multiple of %i bytes (was %i)", AQUAHASH_HASH_BYTES, cache_size);
        PyErr_SetString(PyExc_ValueError, error_message);
        return 0;
    }

    aquahash_params params;
    params.cache_size = (size_t) cache_size;
    params.full_size = (size_t) full_size;
    aquahash_cache cache;
    cache.mem = (void *) cache_bytes;
    void *mem = malloc(params.full_size);
    aquahash_compute_full_data(mem, &params, &cache);
...
hashimoto_light(PyObject *self, PyObject *args) {
    ...
    struct aquahash_light *s;
    s = calloc(sizeof(*s), 1);
    s->cache = cache_bytes;
    s->cache_size = cache_size;
    s->block_number = block_number;
    struct aquahash_h256 *h;
    h = calloc(sizeof(*h), 1);
    for (int i = 0; i < 32; i++) h->b[i] = header[i];
    struct aquahash_return_value out = aquahash_light_compute(s, *h, nonce);
...
hashimoto_full(PyObject *self, PyObject *args) {
    ...
    aquahash_return_value out;
    aquahash_params params;
    params.full_size = (size_t) full_size;
    aquahash_full(&out, (void *) full_bytes, &params, (aquahash_h256_t *) header, nonce);
    return Py_BuildValue("{" PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT ", " PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT "}",
                         "mix digest", &out.mix_hash, 32,
                         "result", &out.result, 32);
}
...
mine(PyObject *self, PyObject *args) {
    ...
    do {
        aquahash_full(&out, (void *) full_bytes, &params, (const aquahash_h256_t *) header, nonce++);
        // TODO: disagrees with the spec https://github.com/ethereum/wiki/wiki/Aquahash#mining
    } while (!aquahash_check_difficulty(&out.result, (const aquahash_h256_t *) difficulty));
...
get_seedhash(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "k", &block_number))
        return 0;
    if (block_number >= AQUAHASH_EPOCH_LENGTH * 2048) {
        char error_message[1024];
        sprintf(error_message, "Block number must be less than %i (was %lu)", AQUAHASH_EPOCH_LENGTH * 2048, block_number);
...
static PyMethodDef PyaquahashMethods[] =
        {
...
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef PyaquahashModule = {
...
PyMODINIT_FUNC PyInit_pyaquahash(void) {
    PyObject *module =  PyModule_Create(&PyaquahashModule);
    // Following Spec: https://github.com/ethereum/wiki/wiki/Aquahash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) AQUAHASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) AQUAHASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) AQUAHASH_DATASET_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "CACHE_BYTES_INIT", (long) AQUAHASH_CACHE_BYTES_INIT);
    PyModule_AddIntConstant(module, "CACHE_BYTES_GROWTH", (long) AQUAHASH_CACHE_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "EPOCH_LENGTH", (long) AQUAHASH_EPOCH_LENGTH);
    PyModule_AddIntConstant(module, "MIX_BYTES", (long) AQUAHASH_MIX_BYTES);
    PyModule_AddIntConstant(module, "HASH_BYTES", (long) AQUAHASH_HASH_BYTES);
    PyModule_AddIntConstant(module, "DATASET_PARENTS", (long) AQUAHASH_DATASET_PARENTS);
    PyModule_AddIntConstant(module, "CACHE_ROUNDS", (long) AQUAHASH_CACHE_ROUNDS);
    PyModule_AddIntConstant(module, "ACCESSES", (long) AQUAHASH_ACCESSES);
    return module;
}
#else
PyMODINIT_FUNC
initpyaquahash(void) {
    PyObject *module = Py_InitModule("pyaquahash", PyaquahashMethods);
    // Following Spec: https://github.com/ethereum/wiki/wiki/Aquahash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) AQUAHASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) AQUAHASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) AQUAHASH_DATASET_BYTES_GROWTH);
