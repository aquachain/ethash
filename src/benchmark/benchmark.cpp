...
#include <libaquahash/util.h>
#ifdef OPENCL
#include <libaquahash-cl/aquahash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(aquahash_h256_t const *hash, unsigned size)
{
...
extern "C" int main(void)
{
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		aquahash_return_value hash;
		aquahash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("aquahash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		aquahash_return_value hash;
		aquahash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : aquahash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			aquahash_return_value hash;
			aquahash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			aquahash_return_value hash;
			#ifdef FULL
				aquahash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				aquahash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
	...
	unsigned read_size = AQUAHASH_ACCESSES * AQUAHASH_MIX_BYTES;
